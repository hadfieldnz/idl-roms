;+
; NAME:
;   MGH_ROMS_NAIVE_VECTOR_TO_NCDF
;
; PURPOSE:
;   Get vector data from a ROMS file, carry out a naive (non-
;   time-stamped) tidal analysis and save the results to a netCDF
;   file.
;
; CATEGORY:
;   ROMS
...
;-
pro mgh_roms_naive_vector_to_ncdf, history, file_out, $
   ...
   fmt = '(%"Vector tidal analysis (naive) from ROMS dataset %s")'
         ...
         for j=0,tile_count_y[l]-1 do begin
            for i=0,tile_count_x[k]-1 do begin
               ...
               if (~ has_mask) || mask[ii,jj] then begin
                  case n_vdim of
                     3: begin
                        mgh_naive_vector_analysis, $
                             time, reform(u[i,j,*]), reform(v[i,j,*]), cc, $
                             FREQUENCY=frequency
                        coeff[*,ii,jj] = cc
                     end
                     4: begin
                        for m=0,n_vertical-1 do begin
                           mgh_naive_vector_analysis, $
                                time, reform(u[i,j,m,*]), reform(v[i,j,m,*]), cc, $
                                FREQUENCY=frequency
                           coeff[*,ii,jj,m] = cc
                        endfor
                     end
