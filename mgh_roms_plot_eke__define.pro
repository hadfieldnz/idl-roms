...
;
; PURPOSE:
;   This class generates and displays a graph showing eddy kinetic
;   energy (and other statistics) derived from a series of Hslices
;   of ROMS vector data.
;
; CATEGORY:
;   Ocean models.
...
;
; CALLING SEQUENCE:
;   mgh_new, 'mgh_roms_plot_eke', history, variable
;
...
;-
function Mgh_Roms_Plot_Eke::Init, $
     history, variable, $
   ...
   case n_elements(variable) of
      0: begin
         if ohis->HasVar('u') && ohis->HasVar('v') then begin
            self.variable = ['u','v']
         endif else begin
            self.variable = ['ubar','vbar']
         endelse
      end
      2: self.variable = variable
      else: message, BLOCK='MGH_MBLK_MOTLEY', NAME='MGH_M_WRGNUMELEM', 'variable'
   endcase

   if n_elements(parameter) eq 0 then parameter = 'EKE'

   if n_elements(data_range) eq 0 then begin
      case strupcase(parameter) of
         'EKE': data_range = [-3.5,-1.5]
         'MES': data_range = [0,0.5]
      endcase
   endif
   ...
   grid_u = ohis->HsliceGrid(self.variable[0], ETA_RANGE=eta_range, $
                             LONLAT=lonlat, XI_RANGE=xi_range-[1,0])
   grid_v = ohis->HsliceGrid(self.variable[1], ETA_RANGE=eta_range-[1,0], $
                             LONLAT=lonlat, XI_RANGE=xi_range)

   if grid_u.dims.time ne grid_u.dims.time then $
        message, 'Inconsistent time dimensions'

   ;; Calculate x & y locations for EKE/MES data

   x_val = mgh_stagger(grid_u.x, DELTA=[-1,0])
   y_val = mgh_stagger(grid_u.y, DELTA=[-1,0])
   ...
   if strlen(grid_u.dims.time) eq 0 then $
        message, 'Cannot calculate EKE for variables with no time dimension'
   ...
   msg = ['Getting', self.variable, 'data between records', $
   ...
   if n_elements(title) eq 0 then begin
      fmt = '(%"ROMS %s %s & %s %s-%s d")'
      title = string(FORMaT=fmt, parameter, self.variable, mgh_format_float(time_range))
   endif
   ...
   ;; remains to be seen whether the accuracy of the one-pass method
   ;; is adequate in practice. However with the introduction of
   ;; calculations of MES, I have dropped the one-pass code.

   fac = 1/double(rran)

    ;; The first pass accumulates mean velocities.

    mean_u = fltarr(xran, eran)
    mean_v = fltarr(xran, eran)

    for r=rra0,rra1 do begin
       tmp_u = ohis->HsliceData(self.variable[0], GRID=grid_u, MASK_VALUE=0, $
                                RECORD=r, DEPTHS=depth, LEVELS=level, $
                                SIGMAS=sigma)
       mean_u += fac*mgh_stagger(temporary(tmp_u), DELTA=[-1,0])
       tmp_v = ohis->HsliceData(self.variable[1], GRID=grid_v, MASK_VALUE=0, $
                                RECORD=r, DEPTHS=depth, LEVELS=level, $
                                SIGMAS=sigma)
       mean_v += fac*mgh_stagger(temporary(tmp_v), DELTA=[0,-1])
    endfor

    ;; The second pass accumulates the velocity statistics

    values = fltarr(xran, eran)
    ...
    for r=rra0,rra1 do begin
       ...
       case strupcase(parameter) of
          'EKE': begin
             values += 0.5D0*fac*((mgh_stagger(temporary(tmp_u), DELTA=[-1,0])-mean_u)^2+ $
                                  (mgh_stagger(temporary(tmp_v), DELTA=[0,-1])-mean_v)^2)
          end
          'MES': begin
             values += fac*sqrt((mgh_stagger(temporary(tmp_u), DELTA=[-1,0])-mean_u)^2+ $
                                (mgh_stagger(temporary(tmp_v), DELTA=[0,-1])-mean_v)^2)
          end
	    endcase
    endfor
   ...
   self->Finalize, 'Mgh_Roms_Plot_Eke'

   return, 1

end

; Mgh_Roms_Plot_Eke::Cleanup
;
pro Mgh_Roms_Plot_Eke::Cleanup
...
; Mgh_Roms_Plot_Eke::GetProperty
;
pro Mgh_Roms_Plot_Eke::GetProperty, $
     ...
     PALETTE=palette, PARAMETER=parameter, STYLE=style, VARIABLE=variable, _REF_EXTRA=extra
...
; Mgh_Roms_Plot_Eke::SetProperty
;
pro Mgh_Roms_Plot_Eke::SetProperty, $
...
; Mgh_Roms_Plot_Eke::About
;
pro Mgh_Roms_Plot_Eke::About, lun
...
; Mgh_Roms_Plot_Eke::BuildMenuBar
;
; Purpose:
;   Add menus, sub-menus & menu items to the menu bar

pro Mgh_Roms_Plot_Eke::BuildMenuBar
...
; Mgh_Roms_Plot_Eke::EventMenuBar
;
function Mgh_Roms_Plot_Eke::EventMenuBar, event
...
; Mgh_Roms_Plot_Eke::ExportData
;
pro Mgh_Roms_Plot_Eke::ExportData, values, labels
...
; Mgh_Roms_Plot_Eke::ExportToNcFile
;
pro Mgh_Roms_Plot_Eke::ExportToNcFile, file
   ...
   onc->AttAdd, /GLOBAL, 'history', $
                'Generated by routine MGH_ROMS_Plot_Eke::ExportToNcFile at '+ $
...
; Mgh_Roms_Plot_Eke::PickReport
;
pro Mgh_Roms_Plot_Eke::PickReport, pos, LUN=lun
...
pro Mgh_Roms_Plot_Eke__Define
   ...
   struct_hide, {Mgh_Roms_Plot_Eke, inherits MGH_Window, $
