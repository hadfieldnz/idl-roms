;+
; CLASS NAME:
;   MGH_ROMS_Plot_Cslice
;
; PURPOSE:
;   This class generates and displays a single graph showing a Cslice (as
;   implemented in the MGHromsHistory class) through a ROMS 3D output field.
;
;   Cf MGH_ROMS_Movie_Cslice, which shows an animated sequence of Cslices,
;   and MGH_ROMS_Plot_Hslice, which shows a single Hslice.
;
; CATEGORY:
;   Ocean models.
;   Object graphics
;
; CALLING SEQUENCE:
;   mgh_new, 'MGH_ROMS_Plot_Cslice', File, Var
;
; ARGUMENTS:
;   File (input)
...
;     Written, merging MGH_ROMS_Movie_Cslice and MGH_ROMS_Plot_Hslice.
;-
function MGH_ROMS_Plot_Cslice::Init, history, variable, $
   ...
   var_dims = ohis->VarDims(self.variable)

   has_time = strlen(var_dims.time) gt 0

   if has_time then begin
      if n_elements(record) eq 0 then record = 0
      time_var = ohis->TimeVarName(var_dims.time)
   ...
   if use_bath then begin
      bath = ohis->CtranData('bath', GRID=grid, RECORD=record)
   endif else begin
      bath = grid.h
   endelse
   ...
   if use_zeta then begin
      zeta = ohis->CtranData('zeta', GRID=grid, RECORD=record)
   endif else begin
   ...
   if has_time then begin
      data = ohis->CsliceData(variable, GRID=grid, RECORD=record)
   endif else begin
      data = ohis->CsliceData(variable, GRID=grid)
   endelse
   ...
   ograph = obj_new('MGHgrGraph2D', NAME='ROMS C-slice plot', $
   ...
   self->Finalize, 'Mgh_Roms_Plot_Cslice'

   return, 1

end

; MGH_ROMS_Plot_Cslice::Cleanup
;
pro MGH_ROMS_Plot_Cslice::Cleanup
...
; MGH_ROMS_Plot_Cslice::GetProperty
;
pro MGH_ROMS_Plot_Cslice::GetProperty, $
...
; MGH_ROMS_Plot_Cslice::SetProperty
;
pro MGH_ROMS_Plot_Cslice::SetProperty, $
...
; MGH_ROMS_Plot_Cslice::About
;
;   Print information about the window and its contents
;
pro MGH_ROMS_Plot_Cslice::About, lun
...
; MGH_ROMS_Plot_Cslice::BuildMenuBar

pro MGH_ROMS_Plot_Cslice::BuildMenuBar
...
; MGH_ROMS_Plot_Cslice::EventMenuBar
;
function MGH_ROMS_Plot_Cslice::EventMenuBar, event
...
; MGH_ROMS_Plot_Cslice::ExportData
;
pro MGH_ROMS_Plot_Cslice::ExportData, values, labels
...
; MGH_ROMS_Plot_Cslice::PickReport
;
pro MGH_ROMS_Plot_Cslice::PickReport, pos, LUN=lun
...
; MGH_ROMS_Plot_Cslice::ShowGridPlot
;
; Purpose:
;   Generate a grph showing the slice position(s)

pro MGH_ROMS_Plot_Cslice::ShowGridPlot
...
; MGH_ROMS_Plot_Cslice::UpdateMenuBar
;
pro MGH_ROMS_Plot_Cslice::UpdateMenuBar
...
pro MGH_ROMS_Plot_Cslice__Define
   ...
   struct_hide, {MGH_ROMS_Plot_Cslice, inherits MGH_Window, $
